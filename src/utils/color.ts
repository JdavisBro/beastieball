...
export function bgrDecimalToRgb(bgr: number) {
    return [(bgr & 0xff) / 255, ((bgr >> 8) & 0xff) / 255, ((bgr >> 16) & 0xff) / 255]
}

export function bgrDecimalToHex(bgr: number) {
    return (bgr & 0xff).toString(16).padStart(2, "0") + ((bgr & 0xff00) >> 8).toString(16).padStart(2, "0") + ((bgr & 0xff0000) >> 16).toString(16).padStart(2, "0")
}

export type BeastieColorSet = Array<{color: number, x: number}>;

export function getColorInBeastieColors(x: number, colorset: BeastieColorSet): number[] {
    if (x > 1 || x < 0) {throw Error("Invalid beastie color params.");}
