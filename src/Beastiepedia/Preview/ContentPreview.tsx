...
export default function ContentPreview(props: Props): React.ReactNode {
  ...
  const [colors, setColors] = useState<number[][]>([[30/255, 80/255, 69/255], [32/255, 33/255, 79/255], [52/255, 62/255, 64/255], [245/255, 208/255, 51/255], [220/255, 156/255, 0], [28/255, 84/255, 45/255]]);
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      im.addEventListener("load", () => {if (glRef.current) {setImage(glRef.current, im)}})
    }
  }, [canvasRef.current])
  ...
  function colorChange(change_index: number, color: number[]) {
    setColors((old) => {
      return old.map((value, index) => {
        if (index == change_index) {return color};
        return value;
      })
    })
  } 
  ...
  return (
    <div className={styles.preview}>
      ...
      <canvas className={styles.previewcanvas} width={1000} height={1000} ref={canvasRef} />
      {[0,1,2,3,4,5].map((v) => <input type="color" onChange={(e) => {colorChange(v, hexToRgb(e.target.value))}} />)}
    </div>
  );
}
