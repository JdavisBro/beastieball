...
void main() {
    ...
    if (color.r==0.0 && color.g==0.0 && color.b==0.0) {
        gl_FragColor = vec4(mix(vec3(0.015, 0.098, 0.329), vec3( 0.0, 0.106, 0.412), 0.5 + (0.5*sin(0.0))), 1);
    } else {
      for (int i=0; i < max_colors; i+=1) {
          ...
          if (
              ...
              colorOutHSV.r = mix(ceil(colorOutHSV.r + 0.666) - 0.333, colorOutHSV.r, 0.2); //bluen! 
              colorOutHSV.g = mix(colorOutHSV.g, 0.0, 0.55 + (0.1*sin(0.0))); //desaturate 
              colorOutHSV.b = mix(colorOutHSV.b, 0.0, 0.5); //darken! 
